...
logger_site_folders = '/home/alex/Escritorio/MEGAsync/Python codes/Data E Geofisica avg min/'
# Where are the L0 folders going to be created?
site_folders = '/home/alex/Escritorio/MEGAsync/Python codes/Data E Geofisica avg min/'
# 'Headers.xlsx' file path
headers_file = '/home/alex/Escritorio/MEGAsync/Python codes/XLSX/Headers.xlsx'
# 'Site_variables.xlsx' file path
variables_file = '/home/alex/Escritorio/MEGAsync/Python codes/XLSX/Site_variables.xlsx' 
# Where are the 'Tasks_Meteo_X.xlsx' files stored?
tasks_files = '/home/alex/Escritorio/MEGAsync/Python codes/XLSX/' 
# 'L1_filters.xlsx' file path
L1_filters = '/home/alex/Escritorio/MEGAsync/Python codes/XLSX/L1_filters.xlsx'
#-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*    
...
def missing_data_report(site, Y, M, file_path, skip_rows = [0,1,2,3,4,5,7]):
    ...
    outcoming_route = parts.split('.')[0] + '_' + str(Y) + '-' + str(M) + '_STAT.csv'
...
def col_name(site, df, Timestamp = False, Record = False, sheet_name = 'met'):
    ...
    site_variables = pd.read_excel(variables_file, header = [0,1], sheetname = sheet_name, index_col=0)
...
def merge_files(folder_path, include = [''], exclude = ':', skip_rows = []):
    ...
    for s in onlyfiles:
        ...
        if any(i.lower() in s.lower() for i in include) and exclude.lower() not in s.lower():
            ...
            data = pd.read_csv(route,sep=',',encoding='latin1', skiprows = skip_rows, na_values = ['NAN','null','nan',' '])
            # appending the data
...
def save_month(folder_path, site, df, version = 'L0', extension = '.csv', na_representation = 'null', Timestamp = False, Record = False, MDR = True, sheet_name = 'met', area = 'Meteorological'):
    ...
    for group in grouped_year_month:
        ...
        ### missing data report 
        if MDR:
            missing_data_report(site, Y, M, join(folder_path, filename))
...
def L1_filter(site, df, extra_filters = False, sheet_name = 'met'):
    ...
    site_filters = pd.read_excel(L1_filters, header = [0,1], sheetname = 'Filters_' + sheet_name, index_col=0)
...
def df_mean(df, site, rule = '60Min', sheet_name = 'met'):
    ...
    try:
        ...
        df1 = pd.read_csv(route1, sep=',',skiprows=[0,1,2,3,4,5,7], encoding='latin1', na_values=['null','NAN','nan'])     
