...
for site in sites:
    ...
    start_year, start_month = Master.youngest_date(site, sf = path)
    ...
    dates_array = pd.date_range(start = start, end = end + relativedelta(months = 1), freq = 'M').strftime('%Y-%m')  
    # loop for each month in 'dates_array'
    for date in dates_array:
        print(date)
        # 'file_name' is the name of the file (L0 file) to be read           
        file_name = os.path.join(L0, date + '-' + site + '_L0_cael.dat')
        print(file_name)
        # read the data
        ...
        df['E_Avg'][((df.E_Avg - df.E_Avg.shift(1)) == 0.0) | ((df.E_Avg - df.E_Avg.shift(-1)) == 0.0)] = np.nan
        ...
        std60 = Master.df_mean(df, site, sheet_name = 'cael'); std60.columns = ['E_SD']
        # save the L1 data (minute & hour files)
        Master.save_month(os.path.join(L1, 'Minuto'), site, df, version = 'minuto_L1_cael', MDR = False, sheet_name = 'cael', area = 'Electric field intensity')
        Master.save_month(os.path.join(L1, 'Hora'), site, std60, version = 'hora_L1_cael', MDR = False, sheet_name = 'cael', area = 'Electric field intensity')
